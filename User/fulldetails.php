...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    if (isset($_REQUEST['Add'])) {
        $fid = $_POST['fid'];
        header("location:addcust.php?id=$fid");
    }
    ?>
    ...
    <body>
        ...
        if (isset($_REQUEST['id1'])) {
            $fid = $_GET['id'];
            $a = $_REQUEST['id1'];
            $qb = $conn->query("select * from bookingdetail where bdid='$a'");
            $rb = $qb->fetch_assoc();
            $sno = $rb['seatno'];
            $bid = $rb['bookmid'];
            $date = date("Y-m-d");
            $conn->query("insert into cancel(bookid,date1,seatno,fliid) values('$bid','$date','$sno','$fid')");
                                                        ...
                                                        <fieldset>
                                                            <table width="699"  align="center">
                                                                <tr>
                                                                    ...
                                                                    <th width="55" scope="col"><div align="left">EDIT</div></th>
                                                                    <th width="72" scope="col"><div align="left">DELETE</div></th>
                                                                </tr>
                                                                <?php
                                                                $q = "select * from bookingdetail bd,class c where bd.bookmid='$bookmid' and c.claid=bd.claid"; //echo $q; exit;
                                                                $sql = $conn->query($q);
                                                                $num = $sql->num_rows;
                                                                $_SESSION['ts'] = $num;
                                                                for ($i = 1; $i <= $num; $i++) {
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        $sum = $sum + $summ;
                                                                        ?>
                                                                        <td><a href="editcust.php?idd=<?php echo $row['bdid']; ?>">Edit</a></td>

                                                                        <td><a href="?id1=<?php echo $row['bdid']; ?>&id=<?php echo $fid; ?>">delete</a></td>
                                                                    </tr>
                                                                    <?php
                                                                }
                                                                $_SESSION['sum'] = $sum;
